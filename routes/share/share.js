...
// 根据Leonard的id查到图片src，搜索的时候用了
router.get("/share/blogs/GetLeonardById", async (req, res)=>{
	// console.log("1", req.query)
	try {
		await client.connect();
		let db = client.db("funguy")
		let data = await db.collection('shareImage').find({_id: ObjectID(req.query.id)}).project({files: 1}).toArray()
        data = await dataGeShi(data)
		res.send(data)
        client.close()
	} catch(err) {
